...
function clone_module(name, version, recursive, cache, done) {
  ...
  self.get_version_meta(name, version, function(err, meta) {
    ...
    log.info('cloning %s@%s', name, version || 'latest')
    log.info('GET %s', src)
  ...
  function on_response(err, response, body) {
    ...
    log.info({statusCode: response.statusCode}, src)
