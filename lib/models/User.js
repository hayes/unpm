...
function get_user(username, done) {
  return unpm.get('backend').get_user(username, unpm.bind(done))
}

function set_user(username, data, done) {
  return unpm.get('backend').set_user(username, data, unpm.bind(done))
}
...
function hash_pass(s, settings) {
  settings = settings || unpm.get('config').crypto
