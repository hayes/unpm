...
function getPackage(respond, route, unpm, reqHost) {
  var baseUrl = null
  if (!/^http(s)?/.test(reqHost)) {
    // Incoming reqHost doesn't have its protocol in the URL
    // so we need to apply it from the config settings
    baseUrl = url.format(unpm.config.host.protocol + '://' + reqHost)
  } else {
    baseUrl = url.format(reqHost)
  }
